...
function md5block (str) {
    const out = [];
    for (let i = 0; i < 64; i += 4) {
        out[i >> 2] = str.charCodeAt(i) + (str.charCodeAt(i + 1) << 8) + (str.charCodeAt(i + 2) << 16) + (str.charCodeAt(i + 3) << 24);
    }
    return out;
}
...
function md51 (str) {
    ...
    let i;
    for (i = 64; i <= n; i += 64) md5cycle(state, md5block(str.substring(i - 64, i)));
...
function md5 (value) {
    const str = unescape(encodeURIComponent(String(value)));
    return md51(str).map(hex).join('');
}

export default class WebUtils {
    static log (str) { console.log(str); } // eslint-disable-line no-console
    static _fcnF (fcn, value, data = null) {
        if (typeof fcn !== 'function') return;
        if (data == null) fcn(value); else fcn(data);
    }
    static dateNow () { return Date.now(); }
    static timeMD5 () { return md5(String(Date.now()) + ':' + Math.random()); }
    static io_getmd5 (str) { return md5(str); }
    static encodeBase64 (str) { return btoa(str); }
    static decodeBase64 (str) { return atob(str); }
}
